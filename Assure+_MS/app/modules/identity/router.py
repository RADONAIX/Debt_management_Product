"""Identity routes: /auth (login, refresh, logout, change-password, me, permissions)."""
...
async def my_permissions(principal: CurrentUser) -> dict:
    """The current user's effective permission matrix (from their role).

    The Assure+ UI consumes this to drive nav visibility / RBAC.
    """
...
async def get_user(
    ...
    return service.to_user_row(await service.get_user(db, user_id))
...
async def update_user(
    ...
    if user_id == principal.id and payload.status and payload.status.lower() != "active":
...
async def upsert_role(
    ...
    await service.record_audit(
        db, actor=principal.email, actor_id=principal.id, action="Saved role", target=role.code
    )
    return await service.role_row(db, role)
...
async def update_role(
    ...
    await service.record_audit(
        db, actor=principal.email, actor_id=principal.id, action="Updated role", target=role.code
    )
    return await service.role_row(db, role)
...
@roles_router.delete("/{role_id}", response_model=schemas.ActionResult)
async def delete_role(
    ...
    await service.record_audit(
        db, actor=principal.email, actor_id=principal.id, action="Deleted role", target=role_id
    )
    return schemas.ActionResult(ok=True, detail="Role deleted.")
