...
import { CORPORATE_ACCOUNTS } from "@/components/CorporateAccountManagement";
...
import { AlertCircle, PieChart as PieChartIcon, Heart } from "lucide-react";
...
export function KPI({ title, subtitle, value, icon }) {
  return (
    <div
      className="
    ...
    >
      ...
      <div className="flex justify-between items-end mt-2">
        ...
        <div className="w-11 h-11 flex items-end justify-end p-1.5 shadow-md">
          {icon}
        </div>
      </div>
    </div>
...
export default function DunningEngine() {
  // Headline figures come from the same accounts every other screen reads.
  const scopedAccounts = useScopedAccounts();
  const strategyExec = useMemo(() => deriveStrategyExecution(scopedAccounts), [scopedAccounts]);
  ...
  // Risk Segmentation Data (enterprise-specific rules)
  ...
  // Enterprise Escalation Hierarchy
  ...
  const paymentPlanOptions =
    customerType === "enterprise"
      ? [
          {
            type: "Structured Settlement",
            description:
              "Custom payment schedule aligned with customer's fiscal calendar",
            minAmount: 50000,
            maxDuration: 12,
          ...
          {
            type: "Credit Line Conversion",
            description:
              "Convert outstanding debt to extended credit line with negotiated terms",
            minAmount: 100000,
          ...
          {
            type: "Service-for-Payment",
            description:
              "Allow service credits or extended services in lieu of immediate payment",
            approvalLevel: "C-Suite",
            features: [
      ...
      : [
          {
            type: "Standard Installment",
            description: "Fixed monthly payments over agreed period",
            maxDuration: 6,
            features: ["Auto-debit setup", "Payment reminders"],
          },
          {
            type: "Partial Payment Plan",
            description: "Pay what you can now, rest over time",
            features: ["Flexible schedule", "No penalties"],
          },
        ];

  // Campaign Metrics based on customer type
  const campaignMetrics =
    customerType === "enterprise"
      ? [
          ...
          {
            label: "Account Manager Satisfaction",
            value: "4.6/5",
            icon: Users,
            color: "text-success",
          },
      ...
      : [
          {
            label: "Accounts in Strategy",
            value: formatCount(strategyExec.accountsEnrolled),
            subtitle: "Delinquent accounts enrolled",
            icon: Users,
            color: "text-primary",
          ...
          },
          {
            ...
            value: `${strategyExec.successRatePct}%`,
            subtitle: "Promises honoured",
            icon: TrendingUp,
            color: "text-success",
          ...
          },
          {
            ...
            value: `${strategyExec.avgResolutionDays} days`,
            subtitle: "Case open to close",
            icon: Clock,
            color: "text-info",
          ...
          },
          {
            ...
            value: formatMoney(strategyExec.totalRecovered),
            subtitle: "This cycle",
            icon: DollarSign,
            color: "text-success",
            bgColor: "hsl(var(--success))",
          },
          {
            ...
            value: String(strategyExec.activeStrategies),
            subtitle: "Dunning stages running",
            icon: Activity,
            color: "text-primary",
            bgColor: "hsl(var(--chart-teal))",
          },
          {
            label: "Contact Response Rate",
            value: `${strategyExec.responseRatePct}%`,
            subtitle: "Attempts that reached someone",
            icon: Target,
            color: "text-warning",
  ...
  // The app shell already supplies the page background, font and outer padding,
  // so this screen only owns its internal column layout.
  const pageStyle = {};
      ...
      <div className="flex flex-col gap-10">
        <div className="space-y-6">
          <PageHeader
            ...
            description={
              customerType === "enterprise"
                ? "Contract-focused collection strategies for enterprise accounts. Risk-based automated communication and collection management."
          ...
          <TabsTrigger value="template-assignment">Template Assignment</TabsTrigger>
          ...
          {customerType === 'enterprise' && (
            <TabsTrigger value="corporate-hierarchy">Corporate Hierarchy</TabsTrigger>
          )}
          <TabsTrigger value="settings">Settings</TabsTrigger>
            ...
            <TabsContent value="campaigns" className="space-y-6">
              ...
              >
                {campaignMetrics.map((metric, index) => (
                  <Card key={index} className="p-3.5 hover:border-primary/30 transition-colors">
                    <div className="flex items-start justify-between gap-2">
                      <span className="text-xs font-medium text-muted-foreground leading-tight">
                        {metric.label}
                      </span>
                      <span
                        className="h-7 w-7 rounded-lg shrink-0 flex items-center justify-center text-white"
                        style={{ background: metric.bgColor || "hsl(var(--primary))" }}
                      >
                        <metric.icon className="w-3.5 h-3.5" />
                      </span>
                    </div>

                    <div className="text-xl font-semibold tracking-tight tabular-nums text-foreground mt-2">
                      {metric.value}
                    </div>
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mb-4">
                  Response, delivery and recovery by outreach channel
                </p>
                <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3">
                  {strategyExec.channels.map((ch) => {
                    const Icon = CHANNEL_ICONS[ch.channel] ?? MessageSquare;
                    return (
                      <Card key={ch.channel} className="p-3.5">
                        <div className="flex items-start justify-between gap-2">
                        ...
                        <div className="text-xl font-semibold tabular-nums text-foreground mt-2">
                          {ch.successRatePct}%
                        </div>
                        ...
                        <div className="h-1 w-full rounded-full bg-muted overflow-hidden mt-2">
                          <div
                            ...
                            style={{ width: `${Math.min(ch.successRatePct, 100)}%` }}
                          />
                        </div>

                        <dl className="grid grid-cols-2 gap-2 mt-3 pt-2.5 border-t border-border">
                          <div>
                            <dt className="text-[11px] text-muted-foreground">Recovery</dt>
                            <dd className="text-sm font-semibold tabular-nums text-foreground">
                              {formatMoney(ch.collected)}
                            </dd>
                          </div>
                          <div>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Monitor recovery strategies effectiveness by risk segment
                      ...
                      {campaigns.map((campaign) => {
                        ...
                        return (
                          <TableRow key={campaign.id}>
                            ...
                            {/* <TableCell> */}
                              {/* Recovery per dollar spent reads as a multiple —
                                  a raw % here balloons to four digits. */}
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Detailed breakdown of communication channel effectiveness
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-3">
                              <Checkbox
                                checked={selectedRiskSegments.includes(
                                  segment.id
                                )}
                                onChange={() => {}}
                              />
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Custom rules from Risk Segmentation
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-3">
              ...
              {customRules.length > 0 && (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Your recently created rules - ready for template
                      assignment
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                            ...
                            {allRiskRules
                              ...
                              .map((rule) => (
                                <Badge
                                  ...
                                  variant={getRiskBadgeVariant(rule.riskBand)}
                                >
                                  {rule.name} ({rule.eligibleCustomers}{" "}
                                  eligible)
                                </Badge>
                              ))}
                          </div>
                                ...
                                <DialogTrigger asChild>
                                  <Button
                                    ...
                                    onClick={() => {
                                      ...
                                      const firstCustomer =
                                        eligibleCustomers.find((customer) =>
            ...
            <TabsContent value="ab-testing" className="space-y-6">
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">
                      {
                        currentTests.filter((t) => t.status === "running")
                          .length
                      }
                    </div>
                    <p className="text-xs text-muted-foreground">
                            ...
                            >
                              ...
                              <div className="space-y-2 text-sm">
                                ...
                                <div className="flex justify-between">
                                  <span>Recovery:</span>
                                  <span>
                                    ${variant.recovery.toLocaleString()}
                                  </span>
                                </div>
                                <div className="flex justify-between">
                                  ...
                                  <span>
                                    $
                                    {(variant.cost / variant.responses).toFixed(
                                      2
                                    )}
                                  </span>
                                </div>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Test different communication channels and templates to
                    optimize your dunning strategy
                  </CardDescription>
                </CardHeader>
                          ...
                          <div className="text-2xl font-bold text-primary">
                            {Math.ceil(
                              Math.max(
                                160,
                                Math.ceil(
                                  (selectedCustomers.length || 200) * 0.8
                                )
                              ) / testVariants.length
                            )}
                          </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              <div className="space-y-1">
                                ...
                                <p className="text-lg font-semibold">
                                  {simulationResults.timeToResolution.toFixed(
                                    1
                                  )}{" "}
                                  days
                                </p>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    <div>
                      <Label>Recovery Strategy Type</Label>
            ...
            {/* TAB: Corporate Hierarchy (Enterprise Only) */}
            {customerType === "enterprise" && (
              <TabsContent value="corporate-hierarchy" className="space-y-6">
                {/* Escalation Hierarchy */}
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      <Users className="w-5 h-5" />
                      Multi-Stakeholder Escalation Hierarchy
                    </CardTitle>
                    <CardDescription>
                      Automated escalation path with increasing stakeholder
                      involvement
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      {escalationHierarchy.map((level, index) => (
                        <div key={level.level} className="relative">
                          <div className="flex items-start gap-4 p-4 border rounded-lg bg-card hover:bg-muted/50 transition-colors">
                            ...
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                ...
                                <span>Template: {level.template}</span>
                              </div>
                              {(level.ccAccountManager ||
                                level.ccPrevious ||
                                level.ccLegal) && (
                          ...
                          {index < escalationHierarchy.length - 1 && (
                            <div className="absolute left-[18px] top-[72px] w-0.5 h-8 bg-border"></div>
                          )}
                        </div>
                      ))}
                    </div>
                ...
                {/* Corporate Account Dunning Strategy */}
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                    ...
                    <CardDescription>
                      Manage dunning across parent companies and their
                      subsidiaries
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                        ...
                        >
                          <div className="flex justify-between items-start">
                            <div>
                              ...
                              <p className="text-sm text-muted-foreground mt-1">
                                {account.subsidiaryAccounts.length} subsidiaries
                                • Total Outstanding: $
                            ...
                            <Badge
                              variant={
                                account.paymentStatus === "critical"
                                  ? "destructive"
                            ...
                            <Select defaultValue="consolidated">
                              ...
                              <SelectContent>
                                <SelectItem value="consolidated">
                                  Consolidated - Single communication to parent
                                </SelectItem>
                                <SelectItem value="split">
                                  Split - Separate communications per subsidiary
                                </SelectItem>
                                <SelectItem value="hybrid">
                                  Hybrid - Parent CC'd on subsidiary
                                  communications
                                </SelectItem>
                              </SelectContent>
                            </Select>
                          ...
                          <details className="text-sm">
                            <summary className="cursor-pointer font-medium flex items-center gap-2 hover:text-primary">
                              ...
                              {account.subsidiaryAccounts.length})
                            </summary>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      ...
                      Enterprise Payment Plan Options
                    </CardTitle>
                    <CardDescription>
                      Flexible payment arrangements for corporate accounts
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                        ...
                        >
                          <div className="flex items-start justify-between">
                            ...
                            {(option.approvalRequired ||
                              option.requiresExecutiveApproval) && (
                            ...
                            <div className="flex items-center space-x-3">
                              ...
                              <div>
                                ...
                                <div className="flex items-center space-x-2 text-sm text-muted-foreground">
                                  <Badge
                                    variant={getStageColor(template.stage)}
                                  >
                                    {template.stage}
                                  </Badge>
                                  <span>•</span>
                                  ...
                                  <span>•</span>
                                  <span>
                                    {template.assignedToSegments.length}{" "}
                                    segment(s)
                                  </span>
                                </div>
                              </div>
                          ...
                          <div className="flex items-center space-x-2 text-xs text-muted-foreground">
                            ...
                            {template.assignedToSegments.map((segment) => (
                              <Badge key={segment} variant="outline">
                                {riskSegments.find((s) => s.id === segment)
                                  ?.name || segment}
                              </Badge>
                            ))}
                          </div>
                      ...
                      {customerType === "enterprise" ? (
                        <>
                          <Card className="p-4">
                            ...
                            <p className="text-xs text-muted-foreground">
                              Contract-based payment reminders with account
                              manager contact
                            </p>
              ...
              {customerType === "enterprise" && (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Configure approval workflows and compliance requirements
                  ...
                  <CardContent className="space-y-6">
                    <div>
                      <Label className="text-sm font-medium mb-3 block">
                        Require Executive Approval For
                      </Label>
                      <div className="space-y-3">
                        <div className="flex items-center space-x-2">
                          <Checkbox id="approval-legal" defaultChecked />
                          <label
                            htmlFor="approval-legal"
                            className="text-sm cursor-pointer"
                          >
                            Legal escalation and formal notices
                          </label>
                        </div>
                        <div className="flex items-center space-x-2">
                          <Checkbox id="approval-payment-plan" defaultChecked />
                          <label
                            htmlFor="approval-payment-plan"
                            className="text-sm cursor-pointer"
                          >
                            Payment plans exceeding $100,000
                          </label>
                        </div>
                        <div className="flex items-center space-x-2">
                          <Checkbox
                            id="approval-service-suspension"
                            defaultChecked
                          />
                          <label
                            htmlFor="approval-service-suspension"
                            className="text-sm cursor-pointer"
                          >
                        ...
                        <div className="flex items-center space-x-2">
                          <Checkbox id="approval-contract-breach" />
                          <label
                            htmlFor="approval-contract-breach"
                            className="text-sm cursor-pointer"
                          >
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="am-notification">
                        Account Manager Notification
                      </Label>
                      <Select defaultValue="all">
                        <SelectTrigger id="am-notification">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="all">
                            All dunning communications
                          </SelectItem>
                          <SelectItem value="escalation">
                            Escalations only (D+7 and beyond)
                          </SelectItem>
                          <SelectItem value="critical">
                            Critical notices only (D+30 and beyond)
                          </SelectItem>
                          <SelectItem value="never">Never notify</SelectItem>
                        </SelectContent>
                      </Select>
                      <p className="text-xs text-muted-foreground">
                        Control when account managers receive copies of dunning
                        communications
                      </p>
                    </div>

                    <div className="space-y-2">
                      <Label htmlFor="white-glove-threshold">
                        White-Glove Service Threshold
                      </Label>
                      <div className="flex items-center gap-2">
                        <DollarSign className="w-4 h-4 text-muted-foreground" />
                        <Input
                          id="white-glove-threshold"
                          type="number"
                          defaultValue="500000"
                      ...
                      <p className="text-xs text-muted-foreground">
                        Contract value threshold for premium white-glove
                        handling
                      </p>
                    </div>
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center justify-between p-3 border rounded-lg">
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              Maintain detailed audit trail for regulatory
                              compliance
                            </p>
                          </div>
                          <Switch defaultChecked />
                        ...
                        <div className="flex items-center justify-between p-3 border rounded-lg">
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              Generate monthly compliance reports automatically
                            </p>
                          </div>
                          <Switch defaultChecked />
                        </div>
                        <div className="flex items-center justify-between p-3 border rounded-lg">
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              Automatically archive/delete data per retention
                              policy
                            </p>
                          </div>
                          <Switch />
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="sla-breach-alert">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Alert account managers when SLA compliance falls below
                        this threshold
                      </p>
                    </div>
